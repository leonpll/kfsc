...
import ru.yandex.clickhouse.{BalancedClickhouseDataSource, ClickHouseDataSource}
...
//  var dataSource: ClickHouseDataSource = _
  var dataSource: BalancedClickhouseDataSource = _
...
//    dataSource = new ClickHouseDataSource(url, properties)
  ...
  override def invoke(map: util.Map[String, Any], context: SinkFunction.Context[_]): Unit = {
    ...
    try { //this inserts your data
      ...
      case e: SQLException =>
        LOG.error("invoke SQL Exception" + e.getMessage)
...
//      LOG.error("general insert row time: {}", end-start)
    } catch {
      case e: Exception =>
